...
BBOX_COLOR = '#00FF00'  # Colof of bounding box and caption
BBOX_WIDTH_PX = 4  # Width of bounding box
FONT = 'FreeSansBold.ttf'  # Preinstalled ttf-freefont Ubuntu package
...
class VideoProcessor:
    ...
    animated: bool
    api_response: types.AnnotateVideoResponse
    object_count: int
    video: cv.VideoCapture = None
    cell_size: ImageSize
    grid_size: ImageSize
    ...
    def get_api_response(self) -> types.AnnotateVideoResponse:
        def convert_dict_time_offsets(d: Dict):
            """ Convert time offsets into protobuf 'x.ys' serialization """
            for key, value in d.items():
                if key.endswith('time_offset'):
                    seconds = value.get('seconds', 0)
                    nanos = value.get('nanos', 0)
                    d[key] = f'{seconds}.{nanos:09}s'
                elif isinstance(value, dict):
                    convert_dict_time_offsets(value)
                elif isinstance(value, list):
                    for sub_dict in value:
                        convert_dict_time_offsets(sub_dict)
        annot_dict = self.storage.annot_dict.copy()
        convert_dict_time_offsets(annot_dict)
        return json_format.ParseDict(annot_dict, types.AnnotateVideoResponse())

    def gen_video_objects_filtered(self):
        for obj in self.api_response.annotation_results[0].object_annotations:
            if all([MIN_CONFIDENCE <= obj.confidence,
                    MIN_FRAMES <= len(obj.frames)]):
                yield obj
    ...
    def render_object_animations(self):
        for obj_idx, obj in enumerate(self.gen_video_objects_filtered()):
            image_gen = (self.get_frame_with_overlay(obj, frame)
    ...
    def get_frame_with_overlay(self, obj, frame) -> PilImage:
        ...
        def add_bounding_box(image: PilImage) -> PilImage:
            ...
            r = (int(image_w * bb.left + 0.5), int(image_h * bb.top + 0.5),
                 int(image_w * bb.right + 1.5), int(image_h * bb.bottom + 1.5))
    ...
    def upload_image(self, frames: Union[PilImage, Iterator[PilImage]], filename_suffix: str):
        mem_file = BytesIO()
        ...
        if isinstance(frames, PilImage):
            ...
            save_parameters = dict(save_all=True,
